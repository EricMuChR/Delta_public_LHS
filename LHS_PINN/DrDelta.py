...
class robot(am.arm):
    ...
    # ================= 新增：PINN 数据采集专用接口 =================
    def get_current_model_angles(self):
        """
        静默读取当前关节模型角度，用于数据采集。
        返回: [theta1, theta2, theta3] 或 None
        """
        servo_angle_list = self.read_joints() # 读取物理电机角度
        if servo_angle_list:
            # 转换为几何模型角度
            return self.servo_to_model(servo_angle_list=servo_angle_list)
        return None
